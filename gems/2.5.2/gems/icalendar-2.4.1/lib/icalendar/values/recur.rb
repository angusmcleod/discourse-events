      ...
      def parse_fields(value)
        {
          frequency:
            (
              if value =~ /FREQ=(SECONDLY|MINUTELY|HOURLY|DAILY|WEEKLY|MONTHLY|YEARLY)/i
                $1.upcase
              else
                nil
              end
          ...
          by_second:
            (value =~ /BYSECOND=(#{NUM_LIST})(?:;|\z)/i ? $1.split(",").map { |i| i.to_i } : nil),
          by_minute:
            (value =~ /BYMINUTE=(#{NUM_LIST})(?:;|\z)/i ? $1.split(",").map { |i| i.to_i } : nil),
          by_hour:
            (value =~ /BYHOUR=(#{NUM_LIST})(?:;|\z)/i ? $1.split(",").map { |i| i.to_i } : nil),
          by_day: (value =~ /BYDAY=(#{WEEKDAY}(?:,#{WEEKDAY})*)(?:;|\z)/i ? $1.split(",") : nil),
          by_month_day:
            (value =~ /BYMONTHDAY=(#{MONTHDAY}(?:,#{MONTHDAY})*)(?:;|\z)/i ? $1.split(",") : nil),
          by_year_day:
            (value =~ /BYYEARDAY=(#{YEARDAY}(?:,#{YEARDAY})*)(?:;|\z)/i ? $1.split(",") : nil),
          by_week_number:
            (value =~ /BYWEEKNO=(#{MONTHDAY}(?:,#{MONTHDAY})*)(?:;|\z)/i ? $1.split(",") : nil),
          by_month:
            (value =~ /BYMONTH=(#{NUM_LIST})(?:;|\z)/i ? $1.split(",").map { |i| i.to_i } : nil),
          by_set_position:
            (value =~ /BYSETPOS=(#{YEARDAY}(?:,#{YEARDAY})*)(?:;|\z)/i ? $1.split(",") : nil),
