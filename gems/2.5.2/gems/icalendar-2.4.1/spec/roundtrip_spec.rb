...
describe Icalendar do
  ...
  describe "non-default values" do
    let(:source) do
      File.read File.join(File.dirname(__FILE__), "fixtures", "nondefault_values.ics")
    end
    ...
    it "will output value param on dtstart" do
      expect(
        subject.dtstart.to_ical(subject.class.default_property_types["dtstart"]),
      ).to match /^;VALUE=DATE:20061215$/
    end

    it "will output value param on dtend" do
      expect(
        subject.dtend.to_ical(subject.class.default_property_types["dtend"]),
      ).to match /^;VALUE=DATE:20061215$/
    end
  end
  ...
  describe "sorting date / time events" do
    let(:source) do
      File.read File.join(File.dirname(__FILE__), "fixtures", "two_date_time_events.ics")
    end
    subject { Icalendar::Calendar.parse(source).first.events }
  ...
  describe "non-standard values" do
    ...
    context "lenient parser" do
      ...
      context "saves non-standard fields" do
        ...
        specify do
          expect(
            parsed.custom_property("customfield").first,
          ).to eq "Not properly noted as custom with X- prefix."
        end
        specify do
          expect(
            parsed.custom_property("CUSTOMFIELD").first,
