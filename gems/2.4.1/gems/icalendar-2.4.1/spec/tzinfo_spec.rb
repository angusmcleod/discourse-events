...
describe "TZInfo::Timezone" do
  ...
  describe "daylight recurrence rule" do
    specify do
      expect(
        subject.daylights.first.rrule.first.value_ical,
      ).to eq "FREQ=YEARLY;BYDAY=-1SU;BYMONTH=3"
    end
  end
  ...
  describe "standard recurrence rule" do
    specify do
      expect(
        subject.standards.first.rrule.first.value_ical,
      ).to eq "FREQ=YEARLY;BYDAY=-1SU;BYMONTH=10"
    end
  end
