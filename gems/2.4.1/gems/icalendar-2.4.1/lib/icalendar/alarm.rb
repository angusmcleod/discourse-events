module Icalendar
  class Alarm < Component
    ...
    required_property :description,
                      Icalendar::Values::Text,
                      ->(alarm, description) do
                        alarm.action.downcase == "audio" || !description.nil?
                      end
    required_property :summary,
                      Icalendar::Values::Text,
                      ->(alarm, summary) { alarm.action.downcase != "email" || !summary.nil? }
    required_multi_property :attendee,
                            Icalendar::Values::CalAddress,
                            ->(alarm, attendees) do
                              alarm.action.downcase != "email" || !attendees.compact.empty?
                            end
