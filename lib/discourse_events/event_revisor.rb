      ...
      if @tc.guardian.can_edit_event?(@tc.topic.category)
        ...
        if rsvp
          ...
          going_max_change =
            @tc.record_change(
              "event_going_max",
              @tc.topic.custom_fields["event_going_max"],
              going_max,
            )
        ...
        if start_change || end_change || timezone_change # increment by 1, even if more than one props are changed at once
          @tc.topic.custom_fields["event_version"] = (
            if @tc.topic.custom_fields["event_version"].nil?
              1
            else
              @tc.topic.custom_fields["event_version"] + 1
            end
          )
        end
      end
