...
module OmniEvent
  class EventHash < ::Hashie::Mash; end
  class Builder
    def initialize(&block)
    end
  end
  class << self
    def create_event(provider_type, opts); end
    def update_event(provider_type, opts); end
    def destroy_event(provider_type, opts); end
    def list_events(provider_type, opts)
      raw_events = JSON.parse(
        File.open(
