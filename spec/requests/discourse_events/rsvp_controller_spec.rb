...
describe DiscourseEvents::RsvpController do
  ...
  describe "#add" do
    context "with rsvp enabled" do
      ...
      it "adds user to event going" do
        post "/discourse-events/rsvp/add", params: {
  ...
  describe "#remove" do
    context "with rsvp enabled" do
      before do
        topic.custom_fields['event_going'] = [user.id]
        enable_rsvp
      end

      it "removes user from event going" do
        expect(topic.reload.event_going).to include(user.id)

        post "/discourse-events/rsvp/remove", params: {
          username: user.username,
          topic_id: topic.id,
          type: "going"
        }
        expect(response.status).to eq(200)
        expect(response.parsed_body["success"]).to eq("OK")
        expect(topic.reload.event_going).not_to include(user.id)
      end
    end
  end
  ...
  describe "#users" do
    ...
    context "with rsvp enabled" do
      ...
      it "lists users by rsvp type" do
        get "/discourse-events/rsvp/users", params: {
          type: "going",
          topic_id: topic.id
        }
        expect(response.status).to eq(200)
