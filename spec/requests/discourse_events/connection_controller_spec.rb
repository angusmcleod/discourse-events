...
describe DiscourseEvents::ConnectionController do
  ...
  it "creates connections" do
    put "/admin/events/connection/new.json", params: {
      connection: {
        user: {
  ...
  it "updates connections" do
    ...
    put "/admin/events/connection/#{connection.id}.json", params: {
      connection: {
        client: new_client
      }
    }
  ...
  it "enqueues a connection sync" do
    expect_enqueued_with(job: :discourse_events_sync_connection, args: { connection_id: connection.id }) do
